#!/usr/bin/env python3
"""
Package the Leafmap QGIS plugin for upload to the official QGIS plugin repository.

This script creates a zip file with the following characteristics:
- Root folder renamed from 'leafmap' to the target plugin name
...
def package_plugin(
    ...
    Args:
        source_dir: Path to the qgis_leafmap directory
        output_path: Optional path for the output zip file
        target_name: Name for the root folder in the zip (default: 'qgis_leafmap')
...
def main():
    parser = argparse.ArgumentParser(
        description="Package Leafmap QGIS plugin for repository upload",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output path for the zip file (default: qgis_leafmap-{version}.zip in the same folder as this script)",
    )
    parser.add_argument(
        "--source",
        "-s",
        type=Path,
        default=None,
        help="Source directory (default: qgis_leafmap in the same folder as this script)",
    )
    parser.add_argument(
        "--name",
        "-n",
        type=str,
        default="qgis_leafmap",
        help="Target plugin name for the zip root folder (default: qgis_leafmap)",
    )
    parser.add_argument(
        "--no-version",
        action="store_true",
    ...
    source_dir = args.source if args.source else script_dir / "qgis_leafmap"
