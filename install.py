#!/usr/bin/env python3
"""
Cross-platform installation script for Leafmap QGIS Plugin.
...
def install_plugin(
    source_dir: Path, plugin_dir: Path, plugin_name: str = "qgis_leafmap"
) -> bool:
...
def remove_plugin(plugin_dir: Path, plugin_name: str = "qgis_leafmap") -> bool:
...
def main():
    parser = argparse.ArgumentParser(
        description="Install or remove Leafmap QGIS Plugin"
    )
    parser.add_argument(
        "--remove",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="Plugin folder name in QGIS plugins directory (default: qgis_leafmap)",
    )
    args = parser.parse_args()

    # Get script directory
    script_dir = Path(__file__).parent.resolve()
    source_dir = script_dir / "qgis_leafmap"
